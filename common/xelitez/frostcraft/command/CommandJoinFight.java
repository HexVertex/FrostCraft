...
public class CommandJoinFight extends CommandBase
{
	...
	private double getLowsetPossiblePosition(World world, double x, double y, double z)
	{
		...
		{
			if(world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos), (int)Math.floor(z)) != 0 && Block.blocksList[world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos), (int)Math.floor(z))].getCollisionBoundingBoxFromPool(world, (int)Math.floor(x), (int)Math.floor(ypos), (int)Math.floor(z)) != null)
			{
				if((world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos + 1.0D), (int)Math.floor(z)) == 0 || Block.blocksList[world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos + 1.0D), (int)Math.floor(z))].getCollisionBoundingBoxFromPool(world, (int)Math.floor(x), (int)Math.floor(ypos + 1.0D), (int)Math.floor(z)) == null) && (world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos + 2.0D), (int)Math.floor(z)) == 0 || Block.blocksList[world.getBlockId((int)Math.floor(x), (int)Math.floor(ypos + 2.0D), (int)Math.floor(z))].getCollisionBoundingBoxFromPool(world, (int)Math.floor(x), (int)Math.floor(ypos + 2.0D), (int)Math.floor(z)) == null))
				{
