...
{	
	...
	public void updateConnections(World world, int x, int y, int z)
	{
		...
		for(int i = 0;i < 6;i++)
		{
			...
			if(world.getBlock(x1, y1, z1) instanceof IConnect && ((IConnect)world.getBlock(x1, y1, z1)).getConnectionType() == ConnectionTypes.THERMAL && world.getTileEntity(x1, y1, z1) instanceof IIsSource)
			{
				connected[i] = 2;
				connections += 1;
			}
			else if(world.getBlock(x1, y1, z1) instanceof IConnect && ((IConnect)world.getBlock(x1, y1, z1)).getConnectionType() == ConnectionTypes.THERMAL && world.getTileEntity(x1, y1, z1) instanceof TileEntityThermalPipe)
			{
				connected[i] = 1;
				connections += 1;
			}
			else if(world.getBlock(x1, y1, z1) instanceof IConnect && ((IConnect)world.getBlock(x1, y1, z1)).getConnectionType() == ConnectionTypes.THERMAL)
			{
				connected[i] = 3;
				connections += 1;
			}
	...
	public void check(int id)
	{
		...
		for(int i = 0;i < 6;i++)
		{
			...
			{
				if(!reg.isPipeChecked((TileEntityThermalPipe)worldObj.getTileEntity(x1, y1, z1), id))
				{
					reg.addPipeToCheckQueue(id, (TileEntityThermalPipe)worldObj.getTileEntity(x1, y1, z1));
					checked = true;
				}
			}
		}
